<#
    ...
    (VLC, 7-Zip, Adobe Acrobat Reader, ...) via winget. Dell-only packages
    are skipped automatically on non-Dell hardware.
...
& {
    ...
    # DellOnly packages are skipped on non-Dell hardware unless explicitly
    # requested via $env:ONLY.
    # -----------------------------------------------------------------------
    $Packages = @(
        [pscustomobject]@{ Name = 'VLC media player';              Id = 'VideoLAN.VLC';                          DellOnly = $false }
        [pscustomobject]@{ Name = '7-Zip';                         Id = '7zip.7zip';                             DellOnly = $false }
        [pscustomobject]@{ Name = 'Adobe Acrobat Reader';          Id = 'Adobe.Acrobat.Reader.64-bit';           DellOnly = $false }
        ...
        # [pscustomobject]@{ Name = 'Google Chrome';               Id = 'Google.Chrome';                         DellOnly = $false }
        # [pscustomobject]@{ Name = 'Notepad++';                   Id = 'Notepad++.Notepad++';                   DellOnly = $false }
    )
